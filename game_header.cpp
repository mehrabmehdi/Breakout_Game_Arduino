...
int menu(int16_t vRest) { // Menu Screen
    ...
    while (1) {
        tft.fillRect(29, 75, 65, 13, ILI9341_BLACK);
        menu_option1();
        while (menu_option == 1) {
            int select = digitalRead(SEL);
            ...
            if (v > vPlus) {
                menu_option = 2;
                tft.fillRect(29, 75, 65, 13, ILI9341_CYAN);
                menu_option1();
            }
            if (select == 0) {
                delay(100);
        ...
        tft.fillRect(27, 92, 71, 13, ILI9341_BLACK);
        menu_option2();
        playTone(5000, 50);
        while (menu_option == 2) {
            int select = digitalRead(SEL);
            ...
            if (v < vMinus) {
                menu_option = 1;
                tft.fillRect(27, 92, 71, 13, ILI9341_CYAN);
                menu_option2();
            }
            if (select == 0) {
                delay(100);
...
void scoreboard() {
    tft.fillRect(0, 0, tft.width(), 9, ILI9341_BLACK); // Score bar
    tft.setTextSize(1);
    tft.setTextColor(0xFFFF);
    tft.setCursor(0,0);
    tft.print("Lives: ");
...
void draw_ball(ball_stats *ball) { // Draws ball using ball data
    tft.fillRect(ball->x_coord - (ball->length)/2, ball->y_coord - (ball->length)/2, ball->length, ball->length, ILI9341_MAGENTA);
}

void delete_ball(ball_stats *ball) { // Draws white ball (i.e., deletes old ball)
    tft.fillRect(ball->x_coord - (ball->length)/2, ball->y_coord - (ball->length)/2, ball->length, ball->length, ILI9341_WHITE);
}
void draw_platform(platform_stats *platform) { // Draws platform using platform data
    tft.fillRect((platform->x_coord) - (platform->length)/2, (platform->y_coord) - (platform->width)/2, platform->length, platform->width, ILI9341_BLACK);
}

void delete_platform(platform_stats *platform) { // Draws white platform (i.e., deletes old platform)
    tft.fillRect((platform->x_coord) - (platform->length)/2, (platform->y_coord) - (platform->width)/2, platform->length, platform->width, ILI9341_WHITE);
}
...
int play_game(brick_info* bricks, int16_t hRest, int round) {
    ...
    while(1) {
        ...
        if (lives > 1 && ball.y_coord + (ball.length)/2 >= tft.height() - 1) { // Lost a life
            ...
            tft.setTextSize(1);
            tft.setTextColor(0xFFFF);
            tft.fillRect(0, 0, tft.width(), 9, ILI9341_BLACK); // Blank bar
            tft.setCursor(0,0);
            tft.print(lives);
            tft.print(" life left!");
            delay(500);
...
void check_pause(int16_t initial, platform_stats *platform, int16_t hRest, int8_t platform_allow) {
    if ((!digitalRead(SEL)) || initial) { // Game Pause mechanism
        tft.fillRect(0,0,tft.width(),9,ILI9341_BLACK);
        tft.setTextSize(1);
        tft.setTextColor(0xFFFF);
        tft.setCursor(0, 0);
        if (initial == 1) {
